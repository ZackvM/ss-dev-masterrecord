...
class objlisting { 
  ...
  function simpleorschedule($request, $urirqst) { 
   ...
   if (trim($orrqst[3]) !== "" && trim($orrqst[4]) !== "") { 
       ...
       $orSQL = "SELECT ifnull(pxicode,'ERROR') as pxicode, ifnull(targetind,'') as targetind, ifnull(infcind,0) as informedconsentindicator , if(linkeddonor = 1 or delinkeddonor = 1,'X','-') as linkage, ifnull(pxiini,'NO INITIALS') as pxiinitials, ifnull(pxiage,'') as pxiage, ifnull(pxirace,'') as pxirace, ifnull(pxisex,'') as pxisex, trim(concat(ifnull(pxiage,'-'), '/', ucase(substr(ifnull(pxirace,'-'),1,4)), '/', ifnull(pxisex,'-'))) as ars, ifnull(starttime,'') starttime, ifnull(room,'') room, ifnull(surgeons,'') surgeon, trim(ifnull(proctext,'')) as proceduretext FROM four.tmp_ORListing ors where date_format(listdate,'%Y%m%d') = :ordate and location = :orinstitution order by pxiini";
